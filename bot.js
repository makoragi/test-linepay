...
app.post("/webhook", bot_middleware, (req, res, next) => {
    ...
    req.body.events.map((event) => {
        ...
        // We process payment if user says "購入".
        if (event.type === "message"){
            if (event.message.text === "購入"){
                let product_name = "岩岡さん";
                let reservation = {
                ...
                pay.reserve(reservation).then((response) => {
                    ...
                    //
                    reservation.payType = "PREAPPROVED";
                    //
                    ...
                    let message = {
                        ...
                        altText: `岩岡さんはちょっとするには下記のボタンで決済に進んでください`,
                        template: {
                            type: "buttons",
                            // text: `${product_name}を購入するには下記のボタンで決済に進んでください`,
                            text: `岩岡さんはちょっとするには下記のボタンで決済に進んでください`,
                            actions: [
                                // {type: "uri", label: "LINE Payで決済", uri: response.info.paymentUrl.web},
                                {type: "uri", label: "岩岡さんはちょっと", uri: response.info.paymentUrl.web},
                            ]
                        }
                    }
...
app.get("/pay/confirm", (req, res, next) => {
    ...
    return pay.confirm(confirmation).then((response) => {
        ...
        let messages = [{
            ...
            type: "text",
            // text: "ありがとうございます、購入の決済が完了しました。"
            text: "購入しました。岩岡さんはちょっと。"
        }]
